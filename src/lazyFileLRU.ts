// adapted from https://github.com/phiresky/sql.js-httpvfs/blob/24a51a6e53c145a0b056b2385a1c7a16580b0d43/src/lazyFile.ts
// ... which is ...
...
class defaultCache {
  ...
  set(key: number, value: Uint8Array) {
  ...
  get size() {
    return this.values.filter(function(value) { return value !== undefined }).length;
  }
}
...
export type LazyFileConfig = {
  ...
  /** if defined, use an LRU cache instead of a simple Array object, with this max size (chunks) **/
  LRUSize?: number
};
...
export class LazyUint8Array {
  ...
  private getChunk(wantedChunkNum: number): Uint8Array {
    ...
    // guards above ensure that the cache contains Uint8Array at this point
